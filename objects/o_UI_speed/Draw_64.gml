if(!loading) {
	var mx = mouse_x;
	var my = mouse_y;
	var game_menu = instance_nearest(x,y,o_UI_game_menu);
	...
	for(var i = 0; i < 5; i++) {
		...
		if(mx >= x1 and mx <= x2 and my >= y1 and my <= y2) { //if mouse is hovering on the button
			...
			if(i < 2) { //if button is speed slow or slowest
				...
				if(mouse_check_button_pressed(mb_left) and !instance_exists(game_menu)) {
					if(i == 0) {
	...
	if(mouse_hovering != mouse_hovering_prev and mouse_hovering and !instance_exists(game_menu)) {
